...
class SaleItemController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('RemoveCustomer', 'SetComment', 'DeleteItem', 'AddItem',
                    'EditItem', 'EditItemPrice', 'Index', 'IndexPara', 'AddPayment',
                    'CancelSale', 'CompleteSale', 'Complete', 'SuspendSale', 'DeletePayment',
                    'SelectCustomer', 'AddCustomer', 'Receipt', 'UnsuspendSale', 'EditSale',
                    'Receipt', 'Suspend', 'ListSuspendedSale', 'SetPriceTier','SetGDiscount',
                    'DeleteSale','SetGroup','PrintKitchen','ReceiptKitchen','PrintCustomer',
                    'ReceiptCustomer','ChangeTable','SetDisGiftcard','RemoveGiftcard',
                    'MergeTable','Add','SetZone','SetTable','PrintCloseSale','AjaxRefresh',
                    'AjaxF5Dropdown','KitchenInvoice','ConfirmOrder','cashier','ShowItemsDetail',
                    'EditOrdered'),
                'users' => array('@'),
            ),
    ...
    public function actionSetZone($zone_id,$view='index')
    {
        if (Yii::app()->request->isPostRequest && Yii::app()->request->isAjaxRequest ) {
    ...
    public function actionSetTable($table_id,$view='index')
    {
        if (Yii::app()->request->isPostRequest && Yii::app()->request->isAjaxRequest ) {
    ...
    public function actionAddPayment($view='index')
    {
        if (Yii::app()->request->isPostRequest && Yii::app()->request->isAjaxRequest) {
            $data= array();
    ...
    public function actionSetGroup($view='index')
    {
        if (Yii::app()->request->isPostRequest && Yii::app()->request->isAjaxRequest) {
    ...
    public function actionPrintCustomer($view='index')
    {
        $this->layout = '//layouts/column_receipt';
    ...
    public function actionShowItemsDetail($item_id=null,$view='index')
    {
        $data = $this->sessionInfo();
        ...
        $data['grid_columns'] = ReportColumn::getDetailItemsColumns($view);
        $data['view'] = $view;

        //$model->sale_id = $id;
        $data['data_provider'] = Yii::app()->orderingCart->getCartCashierGrid($item_id);
        ...
        /*if (Yii::app()->request->isAjaxRequest) {
            ...
            echo CJSON::encode(array(
                'status' => 'render',
                'div' => $this->renderPartial('partial/_grid', $data, true, true),
            ));
    ...
    private function reload($view='index',$data=array())
    {
